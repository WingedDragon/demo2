...
public class CommonUtil {
	...
	public static List<String> getRandomEmails(int count){
		List<String> emails = new ArrayList<String>();
		String s = "0123456789abcdefghijkmnopqrstuvwxz";
		StringBuffer sb = new StringBuffer();
		String endPre = "@topsec.com";
		for(int i=0;i<count;i++){
			String beginPre = getRandom(count);
			emails.add(beginPre+endPre);
		}
		return emails;
	}
	...
	public static Map<String,String> getBirthPlaceInfos(){
	...
	public static String getRandomName(){
		List<String> likes = Arrays.asList("java","javascript","c","c++","spring","structs","hibernate","mysql",
				"hadoop","hive","spark","hbase","elasticsearch","redis");

		StringBuffer sb = new StringBuffer();
		int idx = new Random().nextInt(likes.size())+1;
		if(idx >= likes.size()){
			idx = new Random().nextInt(likes.size()/2)+1;
		}
		sb.append(likes.get(idx));
		return sb.toString();
	}

	public static String getRandomLikes(int count){
		List<String> likes = Arrays.asList("java","javascript","c","c++","spring","structs","hibernate","mysql",
				"hadoop","hive","spark","hbase","elasticsearch","redis");
		...
		for(int i=0;i<count;i++){
			...
			sb.append(likes.get(idx)).append(" ");
		}
		return sb.toString();
	}


	public static List<String> getRandomLikes2(int count){
		List<String> likes = Arrays.asList("java","javascript","c","c++","spring","structs","hibernate","mysql",
				"hadoop","hive","spark","hbase","elasticsearch","redis");

		List<String> loves = Lists.newArrayList();
		for(int i=0;i<count;i++){
			int idx = new Random().nextInt(likes.size())+1;
			if(idx >= likes.size()){
				idx = idx/2;
			}
			loves.add(likes.get(idx));
		}
		return loves;
	}
	...
	private static List<String> getNations(){
		//11 北京   12 天津   13 河北   14 山西 15 内蒙   21 辽宁   22 吉林   23 黑龙江
		List<String> nations = Arrays.asList("11","12","13","14", "15","21","22","23");
		return nations;
	}

	public static String getRandomNation(){
		List<String> nations = getNations();
		int nationIdx = new Random().nextInt(nations.size());
		String nation = nations.get(nationIdx);
		return nation;
	}


	public static String getRandomInfo(String name){
		...
		int nationIdx = new Random().nextInt(nations.size());
		String nation = nations.get(nationIdx);
		maps.put("nation",nation);

		//地址
		maps.put("addr",nation+"_"+name);
		maps.put("email",name+"@qq.com");
	...
	public static Map<String,String> getRandomFamily(String name){
		Map<String,String> maps = Maps.newHashMap();

		List<String> relations = Arrays.asList("father","mother","sister","brother", "friend","worker");
		int size = relations.size();
		int count = new Random().nextInt(relations.size()) + 1;
		if(count > size){
			count = size;
		}
		...
		for(int i=0;i<count;i++){
			...
			String relation = relations.get(idx);
			maps.put(relation, getRandom(2));
		}
	...
	public static String[] getRandomLikes4Array(int count){
		List<String> likes = Arrays.asList("java","javascript","c","c++","spring","structs","hibernate","mysql",
				"hadoop","hive","spark","hbase","elasticsearch","redis");
		...
		for(int i=0;i<count;i++){
			...
			result[i] = likes.get(idx);
		}
		return result;
	}

	public static List<String> getRandomLikes4List(int count){
		List<String> likes = Arrays.asList("java","javascript","c","c++","spring","structs","hibernate","mysql",
				"hadoop","hive","spark","hbase","elasticsearch","redis");
		...
		if(idx + count < length){
			result = likes.subList(idx, idx+count);
		}else{
			result = likes.subList(0,count);
		}
		return result;
	}
	...
	public static String getRandomDate(int type, int range){
		...
		cal.add(type, num);
		return formatDate4Def(cal.getTime());
	}
	...
	public static void main(String[] args) throws Exception{
		...
		//String path = "F:/sxt/data/hive";
		//createPersonFile(path);
...
//		Date old = new Date();
//		Date times = getTime(old, diff, type);
//
//		String str = "abc";
//		int total = 9;
//		String fill = "*";
//		String r = fillStr(str,total, fill);


		//Map<String,Object> result = convertProperties2Map("db/mysql.properties");

		//int num = CommonUtil.getRandomNum(2);
