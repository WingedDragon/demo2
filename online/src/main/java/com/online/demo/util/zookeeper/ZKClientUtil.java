package com.online.demo.util.zookeeper;

import com.online.demo.util.common.PropertyUtil;
...
public class ZKClientUtil implements Serializable{
    ...
    static{
        ...
        zkKafkaUrl = zkProperty.getProperty(ReleaseConstant.ZK_CONNECT_KAFKA);
        zkUrl = zkProperty.getProperty(ReleaseConstant.ZK_CONNECT);
        zkBeeRoot = zkProperty.getProperty(ReleaseConstant.ZK_BEE_ROOT);
        zkSessionTimeout = Integer.valueOf(zkProperty.getProperty(ReleaseConstant.ZK_SESSION_TIMEOUT)) * 1000;
        zkConnTimeout = Integer.valueOf(zkProperty.getProperty(ReleaseConstant.ZK_CONN_TIMEOUT)) * 1000;
...
//        System.out.println(datas);
        ...
        //1 写数据
//        List<OffsetRangeData> offsets = OffsetRangeData.create4Temp2Bulk(10);
//        for(OffsetRangeData offset : offsets){
//            String topic = offset.getTopic();
//            int partition = offset.getPartition();
//            long fromOffset = offset.getFromOffset();
//            long untilOffset = offset.getUntilOffset();
//
//            //zkBeeRoot
//            String path = zkBeeRoot + ZK_PATH_SEPTAL + topic + ZK_PATH_SEPTAL + partition;
//            String data = topic + BeeConstant.COMMA + partition + BeeConstant.COMMA + fromOffset + BeeConstant.COMMA + untilOffset;
//
